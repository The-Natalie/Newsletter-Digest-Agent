...
One practical implication of this model is context management discipline: keep always-loaded rules concise, move deep guidance into on-demand files, use specialized agents or skills for research when available, and reset context between planning and implementation when that improves focus and reduces drift.

Warning signs that your context strategy is failing include long drifting conversations, implementation that ignores prior decisions, and repeated hallucinations about file structure or command behavior.
